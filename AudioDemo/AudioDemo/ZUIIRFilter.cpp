...
ZUIIRFilter::ZUIIRFilter(double *b, double *a, int len, int maxCountPerBuffer)
{
...
ZUIIRFilter::ZUIIRFilter(double **sos, double g, int sosRows, int sosCols, int maxCountPerBuffer)
{
    filterType = TYPE_SOS;
    this->g = g;
    this->maxCountPerBuffer = maxCountPerBuffer;
    ...
    for(int i = 0; i < sosRows; i++)
    {
        ...
        shared_ptr<ZUIIRFilter> filter(new ZUIIRFilter(b, a, coefficientCount, maxCountPerBuffer));
        subFilters.push_back(filter);
    }
}

ZUIIRFilter::~ZUIIRFilter()
{
    if(sosInput != nullptr)
    {
        free(sosInput);
...
void ZUIIRFilter::updateX(double x){
    xs.push_front(x);
    xs.pop_back();
}

void ZUIIRFilter::updateY(double y)
{
    ys.push_front(y);
    ys.pop_back();
}

void ZUIIRFilter::process(double *input, double *output, int len)
{
...
void ZUIIRFilter::processNormal(double *input, double *output, int len)
{
...
void ZUIIRFilter::processSOS(double *input, double *output, int len)
{
    ...
    for(shared_ptr<ZUIIRFilter> filter : subFilters)
    {
